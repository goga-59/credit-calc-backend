package ru.creditcalc.backend.exception;
...
public class GenericErrorException extends Exception {
    ...
    public GenericErrorException(Throwable cause) {
        this(HttpStatus.INTERNAL_SERVER_ERROR, "unexpected_error", "An unexpected error was occurred! Try again later.", cause);
    }
    ...
    public static <T> void throwIfViolationsFound(Validator validator, T validationTarget) throws GenericErrorException {
        ...
        if (!violations.isEmpty()) {
            ConstraintViolation<T> first = violations.stream().limit(1).findFirst().orElseThrow();
